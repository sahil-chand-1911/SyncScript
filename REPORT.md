...
classDiagram
    ...
    SocketManager --> DocumentStateManager : delegates
    DocumentStateManager --> OTContext : resolves conflicts
    OTContext --> TransformStrategy : uses
    TransformStrategy <|-- InsertStrategy : inherits
    TransformStrategy <|-- DeleteStrategy : inherits
    DocumentStateManager --> DocumentSubject : updates state
```
...
flowchart LR
    ...
    User --> UC2(Join Document Room)
    User --> UC3(Edit Document)
    User --> UC4(View Edit History)
    
    Owner --> UC5(Share Document)
    Owner --> UC6(Change Roles)
```

### 6.3 Sequence Diagram (Real-Time Edit Workflow)
```mermaid
sequenceDiagram
    actor ClientA
    actor ClientB
    ...
    participant OTContext
    participant DB as MongoDB
    
    ClientA->>SocketManager: Emit "edit" (insert 'x' at pos 5)
    SocketManager->>StateManager: handleEdit(documentId, op)
    StateManager->>OTContext: transform(op against history)
    OTContext-->>StateManager: transformedOp
    StateManager->>DB: appendToHistory(transformedOp)
    StateManager->>SocketManager: broadcast(transformedOp)
    SocketManager->>ClientA: Acknowledge (notifyDirect)
    SocketManager->>ClientB: Receive "remote-edit" (notifyOthers)
    ClientB->>ClientB: Apply transformedOp locally
```
...
erDiagram
    ...
    USER ||--o{ COLLABORATOR : "is assigned"
